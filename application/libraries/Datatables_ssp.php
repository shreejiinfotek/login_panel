...
class Datatables_ssp
{
        ...
        for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
            ...
            for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
                ...
                if ( isset( $column['formatter'] ) ) {
					...
					if(gettype($data[$i][ $columns[$j]['dt'] ])=="string")
					{
                  	 $row[ $column['dt'] ] = utf8_encode($data[$i][ $columns[$j]['dt'] ]);
					}
					else
					{
   ...
   static function limit ( $request, $columns )
        ...
        if ( isset($request['order']) && count($request['order']) ) {
            ...
            for ( $i=0, $ien=count($request['order']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['orderable'] == true ) {
                    ...
                    $orderBy[] = ''.$column['customfilter'].' '.$dir;
                }
            }
            ...
            for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
                ...
                if ($requestColumn['searchable'] == 'true') {
                    if($column['customfilter']!='')
					{
						...
						$globalSearch[] = "".$column['customfilter']." LIKE ".$binding;
					}
                }
            }
        }
     ...
     */
    static function simple($request, $conn, $table, $primaryKey, $columns,$myJoin,$custom_where)
    {
        $bindings = array();
        $db = self::db($conn);
		...
		{
			$new_custom_where = "where ".$custom_where;
		}
		else if($custom_where!='')
		{
			$new_custom_where = " and ".$custom_where;
		}
		else
		{
			$new_custom_where = "";
		...
		if($custom_where!="")
		{
			$new2_custom_where = "where ".$custom_where;
		}
		else
		{
			$new2_custom_where="";
		}
		$explode_new2_custom_where=explode("group by",$new2_custom_where);
        // Total data set length
        $resTotalLength = self::sql_exec($db, "SELECT COUNT(`{$primaryKey}`)
			 FROM   `$table` $myJoin $explode_new2_custom_where[0]" 
        );
